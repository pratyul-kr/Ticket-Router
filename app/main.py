...
# from app.database import Base, engine, get_db
# from app.models import Ticket, Agent, TicketCreate, TicketUpdate, TicketResponse, AgentCreate, AgentResponse
# from app.ai_router import classify_ticket
# from app.embeddings import get_embedding
...
# app = FastAPI(
#     title="Ticket Routing System",
#     description="AI-powered helpdesk ticket router using Groq + PostgreSQL",
#     version="1.0.0",
# )
...
#     2. Find similar past tickets using semantic search (RAG context)
#     3. AI classifies using those similar tickets as examples
#     4. Auto-assign to agent with fewest tickets in that department
#     """
#     # 1. Generate embedding for this ticket
#     text = f"{payload.title}. {payload.description}"
#     embedding = get_embedding(text)

#     # 2. Find top 3 similar past tickets to use as RAG context
#     # We only use tickets that have been classified already (have a category)
#     similar_tickets = (
#         db.query(Ticket)
#         .filter(Ticket.embedding.isnot(None))
#         .filter(Ticket.category.isnot(None))
...
#     # 3. Classify with Groq — passing similar tickets as context
#     ai_result = classify_ticket(payload.title, payload.description, similar_tickets)

#     # 4. Auto-assign to agent with fewest active tickets in matching department
#     agent = (
#         db.query(Agent)
#         .filter(Agent.department == ai_result["category"])
#         .outerjoin(Ticket, (Ticket.assigned_to_id == Agent.id) & (Ticket.status != "resolved"))
...
#     """
#     Search tickets by meaning, not keywords.
#     Example: GET /tickets/search/semantic?q=locked out of account
#     """
#     query_embedding = get_embedding(q)
...
# @app.post("/tickets/{ticket_id}/assign", response_model=TicketResponse)
# def assign_ticket(ticket_id: int, agent_id: int, db: Session = Depends(get_db)):
#     ticket = db.query(Ticket).filter(Ticket.id == ticket_id).first()
#     if not ticket:
#         raise HTTPException(status_code=404, detail="Ticket not found")
